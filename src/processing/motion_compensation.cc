...
bool MotionCompensation::CompensationUsingPoseExtrapolater(common::Time point_time_end,
                                                           const pcl::PointCloud<PointType>& pointcloud,
                                                           pcl::PointCloud<PointType>& pointcloud_compensation,
                                                           processing::PoseExtrapolator* extrapolator_ptr
                                                          )
{
      if(extrapolator_ptr == nullptr)
      {

           std::cout << "--------------MotionCompensation-----------------:IMU not yet initialized." << std::endl;
           return false;
      }

      const common::Time point_time_start =
            point_time_end - common::FromSeconds(1.0/lidar_frequency_);

    //   std::cout << std::setprecision(20)
    //             << "point_time_start:" << common::ToUniversalSeconds(point_time_start)
    //             << "  extrapolator_ptr->GetLastPoseTime():" << common::ToUniversalSeconds(extrapolator_ptr->GetLastPoseTime())
    //             << "  delta_time:" << common::ToUniversalSeconds(point_time_start)- common::ToUniversalSeconds(extrapolator_ptr->GetLastPoseTime())
    //             << std::endl;

      if(point_time_start < extrapolator_ptr->GetLastPoseTime())
      {
          std::cout << "Extrapolator is still initializing." << std::endl;
          return false;
      }
      ...
      for(int i = 0; i < pointcloud_size; i++)
      {
         ...
         {
             scan_id = rounded_angle + (lidar_n_scans_ - 1); 
         }

         if(scan_id > (lidar_n_scans_ - 1) || scan_id < 0 )  
         {
             count--;   
             continue;  
         }
         ...
         if(!is_half_passed)  
         {
           if(current_orientation < start_orientation - M_PI / 2) 
           {
                current_orientation += 2 * M_PI;
           }
           else if (current_orientation > start_orientation + M_PI * 3 / 2)
           ...
           if(current_orientation - start_orientation > M_PI) 
           {
             is_half_passed = true;
           }
         }
         ...
         float real_time_ratio_from_start = (current_orientation - start_orientation) / (end_orientation - start_orientation);
         float real_time_ratio_from_end = (current_orientation - end_orientation) / (end_orientation - start_orientation);


        //  std::cout << "--------------------real_time_ratio_from_end:" << real_time_ratio_from_end 
        //            << "----real_time_ratio_from_start:" << real_time_ratio_from_start
        //            << std::endl;

        common::Time point_time = point_time_end + common::FromSeconds(1.0/lidar_frequency_*real_time_ratio_from_end);

        // std::cout << std::setprecision(20)
        //           << "-------------point_time:" << common::ToUniversalSeconds(point_time)
        //           << "  extrapolator_ptr->GetLastExtrapolatedTime():" << common::ToUniversalSeconds(extrapolator_ptr->GetLastExtrapolatedTime())
        //           << std::endl;

        if(point_time < extrapolator_ptr->GetLastExtrapolatedTime())
        {
            // if(!warned)
            // {
            //     LOG(ERROR)
            //         << "Timestamp of individual range data point jumps backwards from "
            //         << extrapolator_ptr->GetLastExtrapolatedTime() << " to " << time_point;

            //     warned = true;
            // }

            point_time = extrapolator_ptr->GetLastExtrapolatedTime();
        }

        transform::Rigid3f transform_point_time = extrapolator_ptr->ExtrapolatePose(point_time).cast<float>();
        
        Eigen::Isometry3f transformation_point_to_local_map = Eigen::Isometry3f::Identity();
        transformation_point_to_local_map.rotate(transform_point_time.rotation().toRotationMatrix());
        transformation_point_to_local_map.pretranslate(transform_point_time.translation());

        // std::cout << std::setprecision(20)
        //           << "transformation_point_to_local_map x:" <<  transform_point_time.translation()[0]
        //           << "  y:" <<  transform_point_time.translation()[1]
        //           << "  z:" <<  transform_point_time.translation()[2]
        //           << "  point_time:" << common::ToUniversalSeconds(point_time)
        //           << std::endl;

        Eigen::Vector3f point_in_lidar; 
        point_in_lidar << point.x, point.y, point.z;
        Eigen::Vector3f point_in_local_map = transformation_point_to_local_map * point_in_lidar;

        registration::PointType point_compensation;
        point_compensation.x = point_in_local_map[0];
        point_compensation.y = point_in_local_map[1];
        point_compensation.z = point_in_local_map[2];
      ...
      transform::Rigid3f transform_point_end_time = extrapolator_ptr->ExtrapolatePose(point_time_end).cast<float>();
      Eigen::Matrix4f transform_to_lidar = Eigen::Matrix4f::Identity();
      transform_to_lidar.block<3,1>(0,3) = transform_point_end_time.translation();
      transform_to_lidar.block<3,3>(0,0) = transform_point_end_time.rotation().toRotationMatrix();

      pcl::transformPointCloud(pointcloud_compensation, 
                               pointcloud_compensation, 
                               transform_to_lidar.inverse());
