...
{       
    ...
    FilterSensorData(timed_id_pointcloud, 
...
void NdtRegistration::FilterSensorData(const TimedIdPointCloud& timed_id_pointcloud, 
                                       pcl::PointCloud<PointType>::Ptr voxel_filtered_pointcloud_ptr)
{
        ...
        item++)
    {
        ...
        // 删除点云消息中距离过近的点
        if(horizontal_radius > filter_parameter_.min_horizontal_radius)
        {
    ...
    // 体素滤波
    // 声明一个指针变量指向新接收到的这一帧点云数据，这一过程调用了pcl::PointCloud<pcl::PointXYZI>的复制构造函数
...
void NdtRegistration::AddKeyframeToMap()
{
    ...
    if(shift >= mapping_parameter_.min_keyframe_distance)
    {
        ...
        // 目标点云设置，这里设置的目标点云只与当前 map_ptr_指向的点云有关，当map_ptr_指向的点云发生变化时，不会影响ndt内的目标点云
        // 也就是说，这一句直接将 map_ptr_指向的点云数据拷贝走，用于配准
        pcl_ndt_.setInputTarget(map_ptr_);
