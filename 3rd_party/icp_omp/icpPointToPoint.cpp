...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
  ...
  chrono::duration<double> time_cost_t0_t1 = chrono::duration_cast < chrono::duration<double> > (t1-t0);
  std::cout << " get active in fitstep: " << time_cost_t0_t1.count() << endl;
...
double IcpPointToPoint::fitStep_new (double *T,const int32_t T_num,Matrix &R,Matrix &t,const std::vector<std::pair<int32_t, int32_t>> &active) {
...
//       m_kd_tree->n_nearest(query,1,result);
      ...
      // set model point
      p_m.val[i][0] = m_kd_tree->the_data[active[i].second][0]; mum0 += p_m.val[i][0];
      p_m.val[i][1] = m_kd_tree->the_data[active[i].second][1]; mum1 += p_m.val[i][1];
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
    for (i=0; i<nact; i++) {
      ...
      p_m.val[i][0] = m_kd_tree->the_data[active[i].second][0]; mum0 += p_m.val[i][0];
      p_m.val[i][1] = m_kd_tree->the_data[active[i].second][1]; mum1 += p_m.val[i][1];
      p_m.val[i][2] = m_kd_tree->the_data[active[i].second][2]; mum2 += p_m.val[i][2];
...
std::vector< std::pair<int32_t, int32_t> > IcpPointToPoint::getInliers (double *T,const int32_t T_num,const Matrix &R,const Matrix &t,const double indist) {

  // init inlier vector + query point + query result
  vector< pair<int32_t, int32_t> >            inliers;
  std::vector<float>         query(m_dim);
  ...
  if (m_dim==2) {
    ...
    for (int32_t i=0; i<T_num; i++) {
      ...
      if (neighbor[0].dis<indist)
        inliers.push_back(make_pair(i, neighbor[0].idx));
    }
...
#pragma omp parallel for num_threads(12) 
    ...
    for (i=0; i<T_num; i++) {
      ...
      kdtree::KDTreeResultVector neighbor_;
      // transform point according to R|t
      query_[0] = (float)(r00*T[i*3+0] + r01*T[i*3+1] + r02*T[i*3+2] + t0);
      query_[1] = (float)(r10*T[i*3+0] + r11*T[i*3+1] + r12*T[i*3+2] + t1);
      query_[2] = (float)(r20*T[i*3+0] + r21*T[i*3+1] + r22*T[i*3+2] + t2);

      // search nearest neighbor
      m_kd_tree->n_nearest(query_,1, neighbor_);

      // check if it is an inlier
      if (neighbor_[0].dis<indist) {
          mute.lock();
          inliers.push_back(make_pair(i, neighbor_[0].idx));
          mute.unlock();
      }
    }
  }
...
double IcpPointToPoint::getResidual( double *T,const int32_t T_num,const Matrix &R,const Matrix &t,const std::vector<std::pair<int,int>> &active )
{	
// 	if(active.empty()) return 0;
	m_active.clear();
	for (int32_t i=0; i<T_num; i++)
		m_active.push_back(make_pair(i,i));
...
double IcpPointToPoint::getError(double *T,const int32_t T_num,Matrix &R,Matrix &t, double indist) {
	if (indist<=0) {
		m_active.clear();
// 		for (int32_t i=0; i<T_num; i++)
// 			m_active.push_back(make_pair(i,i));
	...
	if(m_active.empty()) return 0;
	int nact = m_active.size();
	double residual = 0;
...
// 			std::vector<float>         result(m_dim);
			...
			query[1] = (float)(r10*T[idx*3+0] + r11*T[idx*3+1] + r12*T[idx*3+2] + t1);
			query[2] = (float)(r20*T[idx*3+0] + r21*T[idx*3+1] + r22*T[idx*3+2] + t2);
// 			result[0] = m_kd_tree->the_data[m_active[i].second][0];
// 			result[1] = m_kd_tree->the_data[m_active[i].second][1];
// 			result[2] = m_kd_tree->the_data[m_active[i].second][2];
			// search nearest neighbor
			m_kd_tree->n_nearest(query,1,result);
// 			double dis = (query[0]-result[0])*(query[0]-result[0]) + (query[1]-result[1])*(query[1]-result[1]) + (query[2]-result[2])*(query[2]-result[2]);
// 			mu.lock();
// 			residual += dis;
			residual += result[0].dis;
// 			mu.unlock();
