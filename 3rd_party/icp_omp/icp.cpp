...
:m_dim(dim), m_max_iter(100), m_min_delta(1e-5) 
{
...
double Icp::fit( double *T,const int32_t T_num,Matrix &R,Matrix &t,double indist/*=-1*/ )
{
	...
	// set active points
// 	vector< pair<int32_t,int32_t> > active;
// 	if (indist<=0) {
//         active.clear();
//         for (int32_t i=0; i<T_num; i++)
//         active.push_back(make_pair(i,i));
// 	} else {
...
// 	}
	...
	double residual = getResidual(T,T_num,R,t,m_active);
	return residual;
}
...
void Icp::fitIterate( double *T,const int32_t T_num,Matrix &R,Matrix &t, double indist /*= -1*/ )
{
	...
	for(iter=0; iter<m_max_iter && delta>m_min_delta; iter++){
		...
		if(m_active.size()==0)
			return;
		delta=fitStep_new(T,T_num,R,t,m_active);
	}
}
